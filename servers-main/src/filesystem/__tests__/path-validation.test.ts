...
describe("Path Validation", () => {
  ...
  it("allows subdirectories", () => {
    const allowed = ["/home/user/project"];
    expect(isPathWithinAllowedDirectories("/home/user/project/src", allowed))
      .toBe(true);
    expect(
  ...
  it("blocks similar directory names (prefix vulnerability)", () => {
    ...
    expect(isPathWithinAllowedDirectories("/home/user/project_backup", allowed))
      .toBe(false);
    expect(isPathWithinAllowedDirectories("/home/user/project-old", allowed))
      .toBe(false);
    expect(isPathWithinAllowedDirectories("/home/user/projectile", allowed))
      .toBe(false);
    expect(isPathWithinAllowedDirectories("/home/user/project.bak", allowed))
      .toBe(false);
  });
  ...
  it("handles multiple allowed directories", () => {
    ...
    expect(isPathWithinAllowedDirectories("/home/user/project1/src", allowed))
      .toBe(true);
    expect(isPathWithinAllowedDirectories("/home/user/project2/src", allowed))
      .toBe(true);
    ...
    expect(isPathWithinAllowedDirectories("/home/user/project2-old", allowed))
      .toBe(false);
  });
  ...
  it("blocks parent and sibling directories", () => {
    ...
    expect(isPathWithinAllowedDirectories("/test/allowed_sibling", allowed))
      .toBe(false);
  ...
  it("handles paths with special characters", () => {
    ...
    expect(isPathWithinAllowedDirectories("/home/user/my-project", allowed))
      .toBe(false);
  });
  ...
  describe("Input validation", () => {
    ...
    it("handles trailing separators correctly", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/project/", allowed))
        .toBe(true);
      ...
      expect(isPathWithinAllowedDirectories("/home/user/project2", allowed))
        .toBe(false);
      expect(isPathWithinAllowedDirectories("/home/user/project2/", allowed))
        .toBe(false);
    });
    ...
    it("handles Windows paths with trailing separators", () => {
      if (path.sep === "\\") {
        ...
        expect(isPathWithinAllowedDirectories("C:\\Users\\project\\", allowed))
          .toBe(true);
        ...
        expect(isPathWithinAllowedDirectories("C:\\Users\\project2\\", allowed))
          .toBe(false);
      }
    });
  });
  ...
  describe("Error handling", () => {
    ...
    it("returns false for relative paths in allowed directories", () => {
      ...
      expect(isPathWithinAllowedDirectories("/absolute/path/file", badAllowed))
        .toBe(false);
    });
  ...
  describe("Unicode and special characters", () => {
    it("handles unicode characters in paths", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/café/file", allowed))
        .toBe(true);
    ...
    it("handles paths with spaces correctly", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/my proj", allowed))
        .toBe(false);
    });
  });
  ...
  describe("Overlapping allowed directories", () => {
    it("handles nested allowed directories correctly", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/anything", allowed))
        .toBe(true);
      expect(
      ...
      expect(isPathWithinAllowedDirectories("/home/other/deep/path", allowed))
        .toBe(true);
    });
  ...
  describe("Validation Tests - Path Traversal", () => {
    ...
    it("blocks traversal in allowed directories", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/safe/file", allowed))
        .toBe(true);
      expect(isPathWithinAllowedDirectories("/home/user/project/file", allowed))
        .toBe(false);
    });
  ...
  describe("Validation Tests - Null Bytes", () => {
    it("rejects paths with null bytes", () => {
      ...
      expect(isPathWithinAllowedDirectories("\x00/home/user/project", allowed))
        .toBe(false);
      expect(isPathWithinAllowedDirectories("/home/user/project/\x00", allowed))
        .toBe(false);
    });
  ...
  describe("Validation Tests - Special Characters", () => {
    it("allows percent signs in filenames", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/project/%41", allowed))
  ...
  describe("Path Normalization", () => {
    ...
    it("handles mixed separators correctly", () => {
      if (path.sep === "\\") {
        ...
        expect(isPathWithinAllowedDirectories("C:/Users\\project/src", allowed))
          .toBe(true);
      }
    });
  });
  ...
  describe("Edge Cases", () => {
    it("rejects non-string inputs safely", () => {
      ...
      expect(isPathWithinAllowedDirectories("/home/user/project", [123 as any]))
        .toBe(false);
      expect(isPathWithinAllowedDirectories("/home/user/project", [{} as any]))
        .toBe(false);
    });
  ...
  describe("Symlink Tests", () => {
    ...
    it("resolves nested symlink chains completely", async () => {
      try {
        ...
        expect(isPathWithinAllowedDirectories(fullyResolvedPath, [allowedDir]))
          .toBe(false);
  ...
  describe("Path Validation Race Condition Tests", () => {
    ...
    it("demonstrates parent directory symlink traversal", async () => {
      ...
      if (!symlinkSupported) {
        console.log(
          "   ⏭️  Skipping parent directory symlink traversal test - symlinks not supported",
        );
        return;
      }
    ...
    it("should prevent race condition between validatePath and file operation", async () => {
      ...
      expect(await fs.access(racePath).then(() => false).catch(() => true))
        .toBe(true);
      ...
      await expect(
        fs.writeFile(racePath, "NEW CONTENT", {
          encoding: "utf-8",
          flag: "wx",
        }),
    ...
    it("should handle symlinks that point within allowed directories", async () => {
      ...
      if (!symlinkSupported) {
        console.log(
          "   ⏭️  Skipping symlinks within allowed directories test - symlinks not supported",
        );
        return;
      }
    ...
    it("demonstrates race condition in read operations", async () => {
      ...
      if (!symlinkSupported) {
        console.log(
          "   ⏭️  Skipping race condition in read operations test - symlinks not supported",
        );
        return;
      }
    ...
    it("verifies rename does not follow symlinks", async () => {
      ...
      // Check what happened
      const symlinkExists = await fs.lstat(targetSymlink).then(() => true)
        .catch(() => false);
      const isSymlink = symlinkExists &&
        (await fs.lstat(targetSymlink)).isSymbolicLink();
