...
- ✅ `src/handlers/tool-handlers.ts` - 添加`webdav_list_directory_with_sizes`工具
- ✅ `src/handlers/tool-handlers.ts` - 添加`formatSize`辅助函数
...
WebDAV MCP Server现在具备了与@modelcontextprotocol/server-filesystem相当的功能集，并在某些方面（如智能编辑、搜索功能）有所超越。所有功能均已实现并经过测试，可以投入生产使用。
